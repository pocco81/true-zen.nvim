...
function M.main(option)
	...
	elseif option == "on" then
		if M.get_status() == "off" then
			M.on()
		else
			print("TrueZen: cannot turn minimalist mode on because it is already on")
		end
	elseif option == "off" then
		if M.get_status() == "on" then
			M.off()
		else
			print("TrueZen: cannot turn minimalist mode off because it is already off")
		end
	end
end
